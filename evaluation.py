...
def evaluate(refs, hypos, labels):
    ...
    for i in tqdm(range(len(refs))):
        ...
        if perturb > 0.4: # 25---nli 0.4 classi
            continue
        success += 1
        ff.append({'org_seq': refs[i], 'adv_seq': hypos[i], 'org_label': labels[i]})
    ...
    import csv 
    writer = csv.writer(open("r.csv", 'w'))
       
    for f in ff[:100]:
        writer.writerow([f['org_seq'], f['adv_seq'], f['org_label']])
    ...
    if args.mode == 'classification':
        ...
        with open(args.eval_file, 'r', encoding='utf8') as f:
            ...
            for line in lines:
                ...
                refs.append(line['org_seq'])
               
                hypos.append(line['adv_seq'])
                if "result" in line:
                    labels.append(int(line['result'][0]))
        evaluate(refs, hypos, labels)